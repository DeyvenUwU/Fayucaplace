...
def mensajes(request):
    ...
    if chat_id:
        ...
        # Manejo de peticiones AJAX para enviar y recibir mensajes
        if request.headers.get('x-requested-with') == 'XMLHttpRequest':
            if request.method == "POST":
                content = request.POST.get("content")
                if content:
                    message = Message.objects.create(chat=selected_chat, sender=usuario, content=content)
                    return JsonResponse({"status": "ok", "message_content": message.content, "sender": message.sender.username})
                return JsonResponse({"status": "error", "message": "Content is empty"}, status=400)
            
            # Para peticiones GET de AJAX, devolvemos los mensajes nuevos
            messages = selected_chat.messages.all().order_by("timestamp").values('sender__username', 'content', 'timestamp')
