...
class SubCategoriaViewSet(viewsets.ModelViewSet):
    """CRUD completo para subcategorías. Soporta filtro por ?categoria=<id>."""
    serializer_class = SubCategoriaSerializer
    permission_classes = [permissions.IsAuthenticatedOrReadOnly]

    def get_queryset(self):
        queryset = SubCategoria.objects.all().select_related('categoria')
        category_id = self.request.query_params.get('categoria')
        if category_id:
...
class PublicacionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if anuncio_isnull is not None:
            queryset = queryset.filter(anuncio__isnull=(anuncio_isnull.lower() == 'true'))

        if articulo_isnull is not None:
            queryset = queryset.filter(articulo__isnull=(articulo_isnull.lower() == 'true'))
